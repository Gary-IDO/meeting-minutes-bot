...
enum SummaryPrompt {
    ...
    遇到與會議主題無關的段落，要先分辨是「真實閒聊」還是「辨識錯誤」，判斷依據是有沒有互動：\
    真實閒聊會有其他人針對那段內容本身接話、一來一往（例如有人問中午吃什麼，\
    就有人回話、有人提議、最後有人把話題拉回正事），這種才算單純的閒聊寒暄，\
    可以直接刪除，不必列入第五節。若從頭到尾只有一個人連續講完，沒有任何人回應那段內容，\
    就算開頭有「說到…」「對了…」之類的銜接語，或後面接著有人回到正題，也不算閒聊，\
    一律依規則 2 列入「待釐清問題與風險」。判斷不出有沒有互動時，就當成辨識錯誤標註；\
    寧可多列一段請使用者回聽，也不要讓辨識錯誤無聲消失。
