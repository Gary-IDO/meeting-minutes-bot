...
enum SummaryPrompt {
    ...
    2. 修正明顯的語音辨識錯字與同音字（例如「在」與「再」、「他」與「她」），但不要改變原意。
    3. 去除口語贅詞（嗯、那個、對對對、就是說）與重複語句，改寫成書面語。
    4. 專有名詞、英文縮寫、型號、數字保持原樣，不要翻譯或改寫。
    5. 若逐字稿過短或內容明顯不是會議（例如只有幾個字、雜音），請在「會議摘要」中說明逐字稿內容不足以產出完整紀錄，其餘章節填「無」。
    6. 直接輸出 Markdown 會議紀錄，不要加任何前言、說明或結語。
    """
