...
辨識語言在設定頁選擇，**一場錄音只用一種語言，中途不會自動切換**。

| 語言 | 狀況 |
|---|---|
| 中文（台灣） | 支援，裝置端執行，預設值 |
| English | 支援，需在設定切換 |
| 中文夾雜英文單字、術語 | 通常辨識得到；整段英語對話會明顯變差 |
| **台語、客語等方言** | **完全不支援** |

Apple 的語音辨識支援 42 個語言，其中沒有台語（清單裡的 `yue_CN` 是粵語）。Apple 在 2025 年與教育部合作把台語漢字放進 iOS，那是**字形顯示與輸入**，不是語音辨識。

**這個限制的危險之處在於它不會報錯。** 辨識器無法回答「聽不懂」，會把台語的聲音硬塞進最接近的國語詞，產生讀起來通順、但內容完全不對的句子（實測例子：「罵完」變「肉圓」、「送你」變「SONY」），而且逐字稿上沒有任何標記區分真假。

因此摘要提示詞已特別要求 Claude：遇到語意明顯不連貫的段落，不要自行補成合理內容、不要據此寫出決議或待辦，改為列在「待釐清問題與風險」並標註「疑為語音辨識錯誤，請回聽錄音確認」。**看到這類標記時請回聽錄音**，錄音檔一定會保留。

若台語是會議的主要語言而非偶爾出現，這個架構就不適用，需要改用數位發展部開源的 [Taiwan Tongues ASR CE](https://github.com/adi-gov-tw/Taiwan-Tongues-ASR-CE)（MIT 授權，專為國台客英混講設計，需自行架在有顯卡的電腦上）。
